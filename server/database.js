...
export function getDashboard() {
  ...
  const stacks = snapshots.map((snapshot, index) => {
    ...
    const incomeBreakdown = calculateIncomeBreakdown(snapshot)
    ...
    const passiveIncomeYtd = snapshots
      ...
      .reduce((sum, candidate) => sum + passiveIncomeFromBreakdown(calculateIncomeBreakdown(candidate), candidate), 0)
